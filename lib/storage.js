...
var datStore = require('dwebx-storage')
 ...
 * By default, uses dwebx-secret-storage to storage secret keys in user's home directory.
 * @param {string|object} storage - Storage for ddrive.
 *   `string` is a directory or file. Directory: `/my-data`, storage will be in `/my-data/.dwebx`.
...
function defaultStorage (storage, opts) {
  ...
  try {
    // Store in .dwebx with secret in ~/.dwebx
